...
function getDynamicContent(){
	...
	console.log(vars)
	fetchdata('https://prod.servicedx.com/eventfeed/event/dynamicform/?event_id='+vars.eventId);
...
var fetchdata = async function (url) {
		...
		if (response.ok) {
			...
			console.log(url);
				...
				$("#titleLogoImage").attr('src', formdef.mainContent.eventInfo.titleLogoImage.attachmentUrl);
			...
			function buildRegisterButton(data){
				...
				if(data.mainContent.eventInfo.registrationStartDate !=null && data.mainContent.eventInfo.registrationEndDate !=null){
					...
					console.log("now time", timenow);
					console.log("reg start date", tS);
					console.log("reg end date", tE);
									
								if (inRange(timenow, tS, tE)){
									console.log("button enabled");
								   var registerButton=`<p class="m-b-0" style="font-size:12px;color:orange">Would you like to join this event ?&nbsp;&nbsp;
					...
					function buildHeader(content){
						...
						if(header.hoster){
							...
							$("#header-logo").append(logo);
                                                ...
                                                console.log("HEADERS...",header.isRegistrationRequired);
						if(header.isRegistrationRequired){
							console.log("HEADERS inside");
							...
							var custom = `<div id="loginControls" class="col-md-auto text-right" style="border:0px solid red;white-space: nowrap;">
												<p class="m-b-0" style="font-size:12px;color:orange">Would you like to join this event ?&nbsp;&nbsp;
													<button type="button" style="color:orange" class="btn btn-primary btn-sm" id="registerButton">
													&nbsp;<i class="fad fa-feather-alt"></i>&nbsp; Register</button> 
												</p>
											</div>`
...
function buildAllSessionAndEvents(row){
		...
		//	var timenow = luxon.DateTime.fromISO(luxon.DateTime.now(), {zone: 'America/Port_of_Spain'}).valueOf();
				...
				"columns": [
					{ row: "sessionName",
						render :  function(data,type,row){
							console.log("values of seconf row", row);
							if (type=="sort"){
							...
							var timenowStartDate = luxon.DateTime.fromISO(row.startDate, {zone: 'UTC' }).valueOf();
							var timenowEndDate = luxon.DateTime.fromISO(row.endDate, {zone: 'UTC' }).valueOf();
							var startTime = new Date(timenowStartDate).toLocaleTimeString("en-US").replace(/:\d{2}\s/,' ');
							var endTime = new Date(timenowEndDate).toLocaleTimeString("en-US").replace(/:\d{2}\s/,' ');
							//console.log(s)
							
  
							console.log("ROW VALUES EVENTNAME", row.eventName);
							console.log("ROW VALUES 2", row);
							...
							outStr += moment(row.startDate).format('MMMM D YYYY') +" "+"-"+ startTime;
							outStr += '<span class="text-muted mb-0">&emsp;to&emsp;</span>';
							outStr += moment(row.endDate).format('MMMM D YYYY')+" "+"-"+ endTime;
							outStr += '</h5>';
							outStr += '</div>';
							...
							outStr += '<h6>'+row.sessionDisplayName+'</h6>';
							outStr += '</a>';
							...
							outStr += '<div>';
							//outStr += '<h4>Interesting ?</h4>';
							//outStr += '<h5>Like to register for this session ?</h5>';
							//outStr += '<p class="mb-3">&nbsp</p>';
							outStr += '';
							outStr += '<button type="button" id='+row.sessionName+'  onclick="registerSession(\'' + row.sessionName + '\',\'' + row.eventId + '\')"; class="btn waves-effect waves-light btn-info"><i class="feather icon-link"></i> Register  for This Session</button>&nbsp;&nbsp;';
			...
			function buildSponserData(mainContent,index){
				...
				mainContent[index].sponser.forEach(function (sponsers) {
						...
						sponsorLogos += `<img style="width:80px" class="img-fluid w-20" src="${sponsers.href}">
						&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp`;
			...
			function buildMainContent(content){
				console.log("build main content",content);
		...
		$(document).on("click", "#feedback-tab", function(){
					...
					var tz = jstz.determine(); // Determines the time zone of the browser client
					var timezone = tz.name(); //For e.g.:"Asia/Kolkata" for the Indian Time.
					var timenow = luxon.DateTime.fromISO(luxon.DateTime.now(), {zone: timezone });
					var currentDateTime = new Date(timenow);
	...
	//	alert(sdxenv+"/eventfeed/invitee/"+vars.eventDepartment+"/"+logedInUserName+"/"+vars.eventId+"/?session_name="+sessionName);
		...
		if(logedInUserName!=null){
			...
			postRegistrationSessionData(sdxenv+"/eventfeed/invitee/"+vars.eventDepartment+"/"+logedInUserName+"/"+eventId+"/?session_name="+sessionName,options,logedInUserName);	
		} else {
			...
			$.ajax({ 'url': sdxenv+'/eventfeed/invite/userdetails?event_id='+vars.eventId+'&user_name='+userID, 'method': "GET", 'contentType': 'application/json'
				...
				$.each(data, function(index, value){
					...
					if (value.sessions){ 
						...
						outStr += '<div class="col text-left pb-3"> <i style="font-size:1.05rem;--fa-primary-color:rgb(225,19,131);--fa-secondary-color:rgb(245,130,31);" class="fad fa-star text-muted f-20"></i>&nbsp;Registered</div>';
						...
						if (isEventLive){
							...
							var startTimeforSession = new Date(timenowStartDateforSession).toLocaleTimeString("en-US").replace(/:\d{2}\s/,' ');;
							var endTimeforSession = new Date(timenowEndDateforSession).toLocaleTimeString("en-US").replace(/:\d{2}\s/,' ');
						...
						outStr += '<p class="mb-3 text-muted">Event : '+value.eventName+'&emsp;Session : '+value.sessions.sessionDisplayName+'</p>';
						outStr += '<p class="mb-0">';
						outStr += moment(value.sessions.startDate).format('MMMM D YYYY') +"-"+" "+startTimeforSession;
						outStr += '<span class="text-muted mb-0">&emsp;to&emsp;</span>';
						outStr += moment(value.sessions.endDate).format('MMMM D YYYY')+"-"+" "+endTimeforSession;
						outStr += '</p>';
						outStr += '</div>';
						outStr += '';
						outStr += '</div>';
